...
- [ ] (민우) 상단 바 아래 카테고리에서 페이지 간 이동이 가능(하위 페이지에도 일괄 적용)
...
- [x] (민우) 리뷰 리스트를 지역별, 구장별, 플랫폼별 필터를 적용하여 조회 가능
- [x] (민우) 각 리뷰 게시글의 제목을 클릭하면 게시글 상세 정보 조회 가능
- [ ] (민우) 각 리뷰 게시글에 좋아요/취소 가능

[리뷰 글 작성]

- [x] (민우) 로그인 한 유저는 게시글 작성 가능
- [x] (민우) 제목은 20자 이하로 제한
- [x] (민우) 본문에 이미지 첨부 가능
- [x] (민우) 본문 내용에 placeholder로 주의사항 고지
- [x] (민우) 제목, 본문 내용에 빈 칸이 존재 할 경우와 지역, 경기장, 플랫폼 카테고리 미 기입시 작성 완료 불가

[리뷰 글 수정/삭제]

- [x] (민우) 본인이 작성한 게시글을 조회 할 경우 수정하기 / 삭제 가능

[리뷰 글 댓글]

- [x] (민우) 로그인 한 유저는 게시글에 댓글 작성 가능

[추가기능]

- [x] (민우) 우수 리뷰 등에 대해 추천을 할 수 있고, 추천수가 높은 글을 월간 hot글? 등으로 상단에 노출시킴.
