...
- [X] (도원) 글 작성자는 상세정보 페이지에서 글 내용 수정을 할 수 있다.
- [ ] (도원) 글 수정 권한이 있는지 확인하여 권한이 없는 경우 수정 버튼이 노출되지 않고, 수정이 불가능하다.
- [ ] (도원) 관리자 혹은 글 작성자는 글을 삭제할 수 있다.
- [X] (도원) 각 게시판에서는 새 홍보글을 작성할 수 있다
- [X] (도원) 새 홍보글 작성 시 카테고리를 선택할 수 있고 카테고리에 맞는 입력 양식이 자동으로 제시된다
- [X] (도원) 홍보글 작성/수정 시 HTML태그를 활용하여 글을 작성할 수 있다.
- [X] (도원) 입력 정보가 부정확하거나 부족할 경우 글이 작성되지 않는다

[신청/조회]

- [X] (도원) 팀페이지에서는 팀 모집, 모집중인 팀에 신청이 가능하다.
- [ ] (도원) 글 작성자는 글에 신청한 정보를 조회할 수 있다.
- [ ] (도원) 글 작성자는 신청자를 수락하거나 거절할 수 있다.
